...
import giezz.jdbc_jfx.tables.Employee;
...
public class Page1Controller {
    ...
    @FXML
    void handleNavButtonActon(ActionEvent event) {
        System.out.println(((Button) event.getSource()).getId());
        switch (((Button) event.getSource()).getId()) {
            case "btnToTableView" -> {
                try {
                    App.changeScene(event, "page-2.fxml", "Title", true);
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
