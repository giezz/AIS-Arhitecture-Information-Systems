    ...
    void handleButtonAction(ActionEvent event) {
        ...
        if (source.equals(btnUpdate)) {
            ...
            if (selectedEmployeeRow != null) {
                EmployeeUpdateController.EMPLOYEE_ID = selectedEmployeeRow.getId_employee();
                EmployeeUpdateController.INN = selectedEmployeeRow.getInn();
                EmployeeUpdateController.DATE_OF_BIRTH = selectedEmployeeRow.getDate_of_birth();
                EmployeeUpdateController.FIRST_NAME = selectedEmployeeRow.getFirst_name();
                EmployeeUpdateController.LAST_NAME = selectedEmployeeRow.getLast_name();
                EmployeeUpdateController.MIDDLE_NAME = selectedEmployeeRow.getMiddle_name();
                EmployeeUpdateController.PHONE = selectedEmployeeRow.getPhone();
                EmployeeUpdateController.EMAIL = selectedEmployeeRow.getEmail();
                Window window = source.getScene().getWindow();
                Modality modality = Modality.WINDOW_MODAL;
                App.openScene(window, modality, "employee-update.fxml", "Update", false);
            }
            else
            ...
            try {
                if (
                    ...
                    EmployeeDAO.create(
                            new Employee(
                                    tfINN.getText(),
                                    dpDateOfBirth.getValue().toString(),
                                    tfName.getText(),
            ...
            Employee selectedEmployeeRow = tvEmployee.getSelectionModel().getSelectedItem();
            EmployeeDAO.delete(selectedEmployeeRow.getId_employee());
            showEmployees();
        }
        else if (btnSearch.equals(source)) {
            /* TODO: написать шаблонизатор для фильтра */
